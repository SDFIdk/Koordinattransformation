    ...
    if (inputEPSG.value !== mapProjection) {
        store.dispatch('trans/get', inputEPSG.value + '/' + mapProjection + '/' + coords[0] + ',' + coords[1])
            .then(() => {
                ...
                setMapMarker(output.v1, output.v2)
            })
    } else {
...
const setMapOnClick = () => {
    olMap.value.on('click', e => {
        ...
        const mouseCoords = olMap.value.getEventCoordinate(e.originalEvent)
        // Transformér kun hvis EPSG-koderne er forskellige
        if (mapProjection !== inputEPSG.value) {
            store.dispatch('trans/get', mapProjection + '/' + inputEPSG.value + '/' + mouseCoords[0] + ',' + mouseCoords[1])
                .then(() => {
                const output = store.state.trans.data
                // Abort hvis fejl
        ...
        // Ellers er koordinaterne ens
            ...
            const output = [parseFloat(mouseCoords[0]), parseFloat(mouseCoords[1]), inputCoords.value[2]]
